...
TC_ITMWME2E_00177 - PC1-single code and effect on cart
    ...
    ${Text_ProductName}    Set Variable    asus-zenfone-c-zc451cg-2805995111188
...
TC_ITMWME2E_00178 - PC3-single code and effect on cart
    [Tags]    regression    ready
    ${epoch}=    Get current epoch time
    ${epoch_short}=    Get current epoch time short
    ${Text_CampaignName}    Set Variable    E2E_${epoch}
    ...
    Login Pcms
    Go To Create Campaign page and Create Campaign    ${Text_CampaignName}    ${Text_Detail}    ${Text_Note}
    Go To Create Promotion page and Create Promotion with Single Code and PC3    ${Text_PromotionName}    ${Text_PromotionCode}    ${Text_Detail}    ${Text_Note}    ${Text_DiscountBath}    ${Text_SingleCode}
    ...    ${Text_Prefix}
    ${validate_CouponCode}    Verify Counpon Code    ${Text_PromotionName}
...
TC_ITMWME2E_00179 - PC1-unique code and effect on cart
    [Tags]    regression    ready
    ${epoch}=    Get current epoch time
    ${epoch_short}=    Get current epoch time short
    ${Text_CampaignName}    Set Variable    E2E_${epoch}
    ...
    Login Pcms
    Go To Create Campaign page and Create Campaign    ${Text_CampaignName}    ${Text_Detail}    ${Text_Note}
    Go To Create Promotion page and Create Promotion with Unique Code and PC1    ${Text_PromotionName}    ${Text_PromotionCode}    ${Text_Detail}    ${Text_Note}    ${Text_DiscountBath}    ${Text_SingleCode}
    ...    ${Text_Prefix}
    ${validate_CouponCode}    Verify Counpon Code    ${Text_PromotionName}
    ${Order_Id&Sale_ID}=    Purchase Item and Verify PC1    ${Text_PromotionName}    ${Text_Email}    ${Text_Name}    ${Text_Phone}    ${Text_Address}
    ...    ${Text_CWName}    ${Text_CWCardNo}    ${Text_CWCCV}    ${Text_ProductName}    ${validate_CouponCode}    ${variance}
...
Temp
    Change status to Picked and Packed with Order ID    100011242    105001246    1234    1234
