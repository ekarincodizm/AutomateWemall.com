...
API_Request Token_028 - To verify that API Request Token return error message when system send invalid Username node
    [Tags]    TC_ALL    Token_028

    ${api_token}=    Point API - Get API Token From Authen    ${api_key}    ${api_secret}

    ${request_body}=    Point API - Create Request Api Request Token    "${api_token}"    "${user_id}"    "${username}worng"    "${password}"    "${partner_id}"

    ${response}=    Point API - Send Http Post Request    ${POINT_API_URL_REQUEST_TOKEN}    ${request_body}    500

    ${remove_node} = 	Create List 	api_token    password
    ${request_body_with_remove_node}=    Point API - Remove Node From Json Data    ${request_body}    ${remove_node}

    ${json_error}=    Set Variable    {}
    ${json_error}=    Point API - Get Error For Expect Data    ${json_error}    cannot_connect_partner
    ${json_error}=    Replace String    ${json_error}    "[]"    ${request_body_with_remove_node}

    Point API - Verify Response Error    ${response}    ${json_error}

API_Request Token_029 - To verify that API Request Token return error message when system send invalid Password node
    [Tags]    TC_ALL    Token_029    Token_test

    ${api_token}=    Point API - Get API Token From Authen    ${api_key}    ${api_secret}

    ${request_body}=    Point API - Create Request Api Request Token    "${api_token}"    "${user_id}"    "${username}"    "${password}wrong"    "${partner_id}"
