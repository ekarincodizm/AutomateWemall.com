...
Create - VIP code - Unlimited - On cart with specific date and time condition if specific date
    ...
    ${CodeCanBeUseTime_Text}    Set Variable    1
    ${Prefix_Text}    Set Variable    THNV
    ...
    @{SpecificDate_Data}=            Create List     ${SpecificDateDataName}    ${SpecificDateDataValue1}

    Login Pcms
    Go To Create Campaign page and Create Campaign    ${CampaignName_Text}    ${Detail_Text}    ${Note_Text}
    Create - VIP code - Unlimited - On cart with specific date and time condition if specific date    ${PromotionName_Text}    ${PromotionCode_Text}
    ...    ${Detail_Text}    ${Note_Text}    ${DiscountBath_Text}    ${CodeCanBeUseTime_Text}
    ...    ${Prefix_Text}    /    /    ${SpecificDate_Data}